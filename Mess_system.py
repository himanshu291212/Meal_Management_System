...
engine.say('welcome to mess managment system')
...
class sp:#funtions related to material ordered for mess 
    ...
    def addToInventory(self, Item_id,name,quantity):
        ...
        if  in_db.empty:
            ...
            in_db = in_db._append(pd.DataFrame([[Item_id,name,quantity]],columns=in_db.columns))
    ...
    def add_items(self, item_id,name,quantity,status):#ordering items from market by the mess incharge and adding the material to supply database i.e. "sp_db.csv"
        ...
        sp_db = sp_db._append(pd.DataFrame([[item_id,name,status,quantity]],columns=sp_db.columns))
        sp_db.to_csv('sp_db.csv')
        return sp_db
...
class comp(cooking):#class binding functions related to regiteration of complains from both incharge and students 
    ...
    def complain(self, Cook_id,complaint):
        ...
        complain_db = complain_db._append(pd.DataFrame([[Cook_id,complaint]],columns=complain_db.columns))
...
class MessOps(comp,sp):
    ...
    def add_stu(self,roll_no,name,creds):#addition of data of newly enrolled student to "st_db.csv" 
        ...
        st_db = st_db._append(pd.DataFrame([[roll_no,name,creds]],columns=st_db.columns))
...
class studOps(comp):
    ...
    def add_order(self, order_no,time,roll_no,order,amt):
        df = self.view_order_db()
        df = df.reset_index()
        df = df._append(pd.DataFrame([[order_no,time,roll_no,order,amt]],columns=df.columns))
...
def mess_op():
    ...
    if(choice == '1'):
        ...
        try:
            ...
            amt = input('Enter the Mess Security Amount: ')
        ...
        if(choice == '1'):
            ...
            while(1):
                ...
                if(more == '0'):
                    ...
                    stud.add_order((prev_oid+1),roll_no,ctime(),items,total_amount)
                    print('You ordered: ')
                    for i in items:
                        print(i)
...
def main():
    while(True):
        ...
        tprint("      Mess operations                ")
        print("                          1.For Superivisor Operations                                      ")
        print("                          2. For Student Operations                                          ")
        print("                          9.for exit from the mess system                                    ")
        ...
        if(choice == "1"):
            ...
            engine.say("thanku for using  this system")
            engine.runAndWait()
            break
